        ...
        try:
            if self.device == "cuda":
                pipe = StableDiffusionXLPipeline.from_pretrained(
                    ...
                    variant="fp16",
                    device_map="cuda"  # Load directly to CUDA device
                )
                
                # Use PyTorch's native scaled_dot_product_attention instead of xformers
                # xformers doesn't support RTX 5090 (compute capability 12.0) yet
                # PyTorch 2.0+ has efficient attention built-in
                if self.verbose:
                    print(f"[ModelRegistry] Using PyTorch native attention (xformers not compatible with RTX 5090)")
            else:
                ...
                pipe = StableDiffusionXLPipeline.from_pretrained(
                    ...
                    torch_dtype=dtype,
                    use_safetensors=True,
                    low_cpu_mem_usage=False  # Disable meta device for CPU
                ).to(self.device)
